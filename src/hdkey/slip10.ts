...
import { Keys, SupportedCurve } from '../types';
...
export function deriveChildKey(parent: Keys, index: number, curve: SupportedCurve): Keys {
  ...
  if (isHardened) {
    ...
    const pub = strategy.getPublicKey(parent.key);
  ...
  while (true) {
    ...
    if (strategy.isValidPrivateKey(IL)) {
      childPriv = strategy.computeChildPrivateKey(IL, parent.key);
  ...
  return { key: childPriv, chainCode: IR, publicKey: pubKey };
}
