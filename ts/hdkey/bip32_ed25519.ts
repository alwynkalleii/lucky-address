...
export function deriveChildKey(parent: Keys, index: number): Keys {
  ...
  if (isHardened) {
    ...
    data[0] = 0x02;
    const pub = ed25519.getPublicKey(parent.privateKey);
    data.set(pub, 1);
  }
  ...
  if (!isHardened) {
    const childPriv = addScalars(parent.privateKey, IL);
    const childPub = ed25519.getPublicKey(childPriv);
    return { privateKey: childPriv, chainCode: IR, publicKey: childPub };
  }
